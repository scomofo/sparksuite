...
test('creates normalized beat grid metadata', function() {
  var grid = timing.createBeatGrid({
    bpm: 120,
    time_signature: '4/4',
    downbeats_s: [0, 2, 4]
  });
...
test('creates transport snapshots with beat metadata', function() {
  var transport = timing.createTransport({
    durationSec: 120,
    positionSec: 5,
    beatGrid: {
      bpm: 120,
...
test('schedules cues within lookahead windows', function() {
  ...
  var cues = timing.scheduleCues([
    { id: 'a', timeSec: 10.02 },
    { id: 'b', timeSec: 11.5 }
  ], transport, { lookaheadSec: 0.1 });
...
test('validates beat grid ordering', function() {
  var validation = timing.validateBeatGrid({
    bpm: 100,
    downbeats_s: [0, 4, 2]
  });
