...
function getLegacyRhythmRuntime(){
  ...
  return {
    active: typeof sc.r("rhythmActive") === "boolean" ? sc.r("rhythmActive") : !!(runtime && runtime.legacyRhythmActive),
    beats: Array.isArray(sc.r("rhythmBeats")) ? sc.r("rhythmBeats") : (runtime && Array.isArray(runtime.legacyRhythmBeats) ? runtime.legacyRhythmBeats : []),
    score: typeof sc.r("rhythmScore") === "number" ? sc.r("rhythmScore") : (runtime && typeof runtime.legacyRhythmScore === "number" ? runtime.legacyRhythmScore : 0),
    combo: typeof sc.r("rhythmCombo") === "number" ? sc.r("rhythmCombo") : (runtime && typeof runtime.legacyRhythmCombo === "number" ? runtime.legacyRhythmCombo : 0),
    maxCombo: typeof sc.r("rhythmMaxCombo") === "number" ? sc.r("rhythmMaxCombo") : (runtime && typeof runtime.legacyRhythmMaxCombo === "number" ? runtime.legacyRhythmMaxCombo : 0),
    startTimeMs: typeof sc.r("rhythmStartTime") === "number" ? sc.r("rhythmStartTime") : (runtime && typeof runtime.legacyRhythmStartTimeMs === "number" ? runtime.legacyRhythmStartTimeMs : 0),
    results: sc.r("rhythmResults") || (runtime ? runtime.legacyRhythmResults : null)
  };
}
...
function rhythmTab(){
  ...
  h+='<div style="display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:16px"><button onclick="act(\'rhythmBpm\',\''+(sc.p("rhythmBpm")-10)+'\')" style="width:32px;height:32px;border-radius:50%;background:var(--input-bg);font-size:18px;font-weight:700;color:var(--text-secondary)">-</button>';
  h+='<div style="text-align:center;min-width:60px"><div style="font-size:24px;font-weight:900;color:var(--text-primary)">'+sc.p("rhythmBpm")+'</div><div style="font-size:10px;color:var(--text-muted)">BPM</div></div>';
  h+='<button onclick="act(\'rhythmBpm\',\''+(sc.p("rhythmBpm")+10)+'\')" style="width:32px;height:32px;border-radius:50%;background:var(--input-bg);font-size:18px;font-weight:700;color:var(--text-secondary)">+</button></div>';
...
function getLegacyRunnerRuntime(D){
  var runtime = window.sparkCore && typeof window.sparkCore.getActiveSessionView === "function"
    ...
    : null;
  runtime = runtime && runtime.runtimeState ? runtime.runtimeState : null;
  var target = sc.r("runnerTarget") || null;
  ...
  return {
    active: typeof sc.r("runnerActive") === "boolean" ? sc.r("runnerActive") : !!(runtime && runtime.legacyRunnerActive),
    score: typeof sc.r("runnerScore") === "number" ? sc.r("runnerScore") : (runtime && typeof runtime.legacyRunnerScore === "number" ? runtime.legacyRunnerScore : 0),
    combo: typeof sc.r("runnerCombo") === "number" ? sc.r("runnerCombo") : (runtime && typeof runtime.legacyRunnerCombo === "number" ? runtime.legacyRunnerCombo : 0),
    maxCombo: typeof sc.r("runnerMaxCombo") === "number" ? sc.r("runnerMaxCombo") : (runtime && typeof runtime.legacyRunnerMaxCombo === "number" ? runtime.legacyRunnerMaxCombo : 0),
    lives: typeof sc.r("runnerLives") === "number" ? sc.r("runnerLives") : (runtime && typeof runtime.legacyRunnerLives === "number" ? runtime.legacyRunnerLives : 0),
    distance: typeof sc.r("runnerDistance") === "number" ? sc.r("runnerDistance") : (runtime && typeof runtime.legacyRunnerDistance === "number" ? runtime.legacyRunnerDistance : 0),
    target: target,
    obstacles: Array.isArray(sc.r("runnerObstacles")) ? sc.r("runnerObstacles") : (runtime && Array.isArray(runtime.legacyRunnerObstacles) ? runtime.legacyRunnerObstacles : []),
    results: sc.r("runnerResults") || (runtime ? runtime.legacyRunnerResults : null)
  };
}
...
function runnerTab(){
  ...
  if(sc.p("runnerHighScore")>0)h+='<div style="margin-bottom:12px;font-size:15px;font-weight:800;color:#FFE66D">&#127942; High Score: '+sc.p("runnerHighScore")+'</div>';
...
function runnerResultsPage(){
  ...
  var runtime = getLegacyRunnerRuntime(D);
  var r=runtime.results;
  var isHigh=r.score>=sc.p("runnerHighScore")&&r.score>0;
  var h='<div class="text-center" style="padding-top:20px"><div style="font-size:56px;animation:bn .6s ease">&#127918;</div>';
...
function buildTab(){
  ...
  if(sc.p("progChords").length===0){
    ...
    for(var i=0;i<sc.p("progChords").length;i++){
      ...
      h+='<div class="prog-block'+(sc.r("progPlaying")&&sc.p("progBeat")===i?" active":"")+'"><div class="del-btn" onclick="act(\'progRemove\',\''+i+'\')">&times;</div>';
  ...
  if(sc.p("progChords").length>0){
    ...
    if(scaleKey&&SCALES[scaleKey]){
      ...
      for(var i=0;i<scaleTypes.length;i++){
        var st=scaleTypes[i],sel=sc.p("selectedScale")===st;
      ...
      h+='</div>';
      var positions=getScaleFrets(scaleKey,sc.p("selectedScale"));
      h+='<div class="flex-center">'+scaleSVG(positions,scaleKey,SCALE_NAMES[sc.p("selectedScale")])+'</div>';
      h+='</div>';
    }
  }
