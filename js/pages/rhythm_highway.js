(function() {
  ...
  function startRhythmHighwayPayload(payload, presetName, launchContext) {
    ...
    var resolvedPresetName = resolveRhythmHighwayPresetName(presetName || sc.r("rhythmHighwayPreset") || payload.enginePreset);
    var resolvedLoopSpec = launchContext.loopSpec || sc.r("rhythmHighwayLoop") || null;
  ...
  function tickRhythmHighway() {
    ...
    if (sc.r("rhythmHighwaySnapshot").finished && !sc.r("rhythmHighwayResult")) {
  ...
  function sparkRhythmHighwayStrum() {
    ...
    var outcome = runtime.engine.handleInput({
      kind: "strum",
      laneMask: sc.r("rhythmHighwayHeldMask") || 0,
  ...
  function rhythmHighwayPage() {
    if (sc.r("rhythmHighwayResult")) return rhythmHighwayResultsPage();
    var snapshot = sc.r("rhythmHighwaySnapshot");
    ...
    h += '</div>';
    if (sc.r("rhythmHighwayLaunchContext") && sc.r("rhythmHighwayLaunchContext").label) {
      h += '<div style="margin-bottom:12px;font-size:11px;color:var(--text-muted);font-weight:700">Focused Drill: ' + escHTML(sc.r("rhythmHighwayLaunchContext").label) + '</div>';
    }
    ...
    for (var fi = 0; fi < laneCount; fi++) {
      var active = maskHasLane(sc.r("rhythmHighwayHeldMask"), fi);
    ...
    h += '<button class="btn" onclick="act(\'' + (sc.r("rhythmHighwayLoop") ? "rhythmHighwayClearLoop" : "rhythmHighwayLoopWindow") + '\')" style="background:' + (sc.r("rhythmHighwayLoop") ? "#4ECDC4" : "var(--input-bg)") + ';color:' + (sc.r("rhythmHighwayLoop") ? "#fff" : "var(--text-secondary)") + '">' + (sc.r("rhythmHighwayLoop") ? "Clear Loop" : "Loop Window") + '</button>';
    h += '<button class="btn" onclick="act(\'back\')" style="background:var(--input-bg);color:var(--text-secondary)">Exit</button>';
    h += '</div>';
    if (sc.r("rhythmHighwayLoop")) {
      h += '<div style="margin-top:10px;font-size:11px;color:#4ECDC4;font-weight:800">Looping ' + escHTML(sc.r("rhythmHighwayLoop").label || "current window") + '</div>';
    }
    if (sc.r("rhythmHighwayFeedback")) {
      h += '<div style="margin-top:12px;font-size:12px;color:var(--text-muted)">' + escHTML(sc.r("rhythmHighwayFeedback")) + '</div>';
    }
    h += '</div>';
    return h;
  }
  ...
  function rhythmHighwayResultsPage() {
    var result = sc.r("rhythmHighwayResult");
    ...
    if (sc.r("rhythmHighwayLaunchContext") && sc.r("rhythmHighwayLaunchContext").exerciseFocus) {
      h += '<div style="font-size:12px;color:var(--text-muted);margin-bottom:10px">Focus: ' + escHTML(String(sc.r("rhythmHighwayLaunchContext").exerciseFocus).replace(/_/g, " ")) + '</div>';
    }
    if (moduleGuidance) {
  ...
  function getRhythmHighwayModuleGuidance(result) {
    var launchContext = sc.r("rhythmHighwayLaunchContext") || null;
    ...
    return instrumentModule.getRhythmGuidance(launchContext.exerciseFocus, result || sc.r("rhythmHighwayResult") || null);
  }

  function cloneRhythmHighwayLaunchContext() {
    if (!sc.r("rhythmHighwayLaunchContext")) return null;
    return JSON.parse(JSON.stringify(sc.r("rhythmHighwayLaunchContext")));
  }
  ...
  function getCurrentAssistPreset() {
    var presetName = resolveRhythmHighwayPresetName(sc.r("rhythmHighwayPreset"));
