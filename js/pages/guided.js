...
function guidedDonePage() {
  ...
  h += '</div></div>';
  var nextRequest = window.sparkCore && typeof window.sparkCore.buildGuidedContinuationRequest === "function"
    ? window.sparkCore.buildGuidedContinuationRequest()
    : null;
  var nextSessionNum = nextRequest && nextRequest.sessionNum != null
    ? parseInt(nextRequest.sessionNum, 10)
    : parseInt(guidedStateRead("guidedSession", null), 10);
