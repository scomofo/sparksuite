...
function sessionStateRead(path, fallback){
  var root = null;
...
function getSparkCoreSessionView(){
  if (!window.sparkCore || typeof window.sparkCore.getActiveSessionView !== "function") return null;
  return window.sparkCore.getActiveSessionView() || null;
}

function getSessionPlayerSnapshot(){
  var view = getSparkCoreSessionView();
  return view && view.player ? view.player : {
    streak: sessionStateRead("streak", 0)
  };
}

function getSessionProgressSnapshot(){
  var view = getSparkCoreSessionView();
  return view && view.progress ? view.progress : {
    chordProgress: sessionStateRead("chordProgress", {})
  };
}

function getSessionToastSnapshot(){
  return sessionStateRead("xpToast", { amount: 10 }) || { amount: 10 };
}
...
function getLegacySessionRuntime(D){
  ...
  return {
    chord: sessionStateRead("currentChord", null) || findInstrumentChordByName(D, runtime && runtime.legacyPracticeChordName),
    timer: typeof sessionStateRead("timer", null) === "number" ? sessionStateRead("timer", null) : (runtime && typeof runtime.legacyPracticeRemainingSec === "number" ? runtime.legacyPracticeRemainingSec : (runtime && typeof runtime.legacyPracticeDurationSec === "number" ? runtime.legacyPracticeDurationSec : 0)),
    timerActive: typeof sessionStateRead("timerActive", null) === "boolean" ? sessionStateRead("timerActive", null) : !!(runtime && runtime.legacyPracticeTimerActive)
  };
}

function getLegacyDrillRuntime(D){
  var runtime = getSparkCoreRuntimeState();
  var drillChords = Array.isArray(sessionStateRead("drillChords", [])) && sessionStateRead("drillChords", []).length ? sessionStateRead("drillChords", []) : [];
  ...
  return {
    chords: drillChords,
    timer: typeof sessionStateRead("drillTimer", null) === "number" ? sessionStateRead("drillTimer", null) : (runtime && typeof runtime.legacyPracticeRemainingSec === "number" ? runtime.legacyPracticeRemainingSec : (runtime && typeof runtime.legacyPracticeDurationSec === "number" ? runtime.legacyPracticeDurationSec : 0))
  };
}
...
function getLegacyDailyRuntime(){
  var runtime = getSparkCoreRuntimeState();
  return {
    timer: typeof sessionStateRead("dailyTimer", null) === "number" ? sessionStateRead("dailyTimer", null) : (runtime && typeof runtime.legacyDailyRemainingSec === "number" ? runtime.legacyDailyRemainingSec : (runtime && typeof runtime.legacyDailyDurationSec === "number" ? runtime.legacyDailyDurationSec : 0)),
    complete: typeof sessionStateRead("dailyComplete", null) === "boolean" ? sessionStateRead("dailyComplete", null) : !!(runtime && runtime.legacyDailyComplete)
  };
}
...
function getSessionMetronomeRuntime(){
  var runtime = getSparkCoreRuntimeState();
  return {
    active: typeof sessionStateRead("metronomeOn", null) === "boolean" ? sessionStateRead("metronomeOn", null) : !!(runtime && runtime.metronomeActive),
    bpm: typeof sessionStateRead("metronomeBpm", null) === "number" ? sessionStateRead("metronomeBpm", null) : (runtime && typeof runtime.metronomeBpm === "number" ? runtime.metronomeBpm : 80),
    beat: typeof sessionStateRead("_metroBeat", null) === "number" ? sessionStateRead("_metroBeat", null) : (runtime && typeof runtime.metronomeBeat === "number" ? runtime.metronomeBeat : 0),
    beatsPerBar: typeof sessionStateRead("_metroBeats", null) === "number" ? sessionStateRead("_metroBeats", null) : (runtime && typeof runtime.metronomeBeatsPerBar === "number" ? runtime.metronomeBeatsPerBar : 4)
  };
}
...
function getSessionChordDetectRuntime(){
  ...
  var runtime = getSparkCoreRuntimeState();
  return {
    active: typeof sessionStateRead("chordDetectOn", null) === "boolean" ? sessionStateRead("chordDetectOn", null) : !!(runtime && runtime.chordDetectActive),
    notes: Array.isArray(sessionStateRead("detectedNotes", [])) ? sessionStateRead("detectedNotes", []) : (runtime && Array.isArray(runtime.chordDetectNotes) ? runtime.chordDetectNotes : []),
    match: typeof sessionStateRead("chordMatch", null) === "number" ? sessionStateRead("chordMatch", null) : (runtime && typeof runtime.chordDetectMatch === "number" ? runtime.chordDetectMatch : -1),
    error: sessionStateRead("chordDetectErr", "") || (runtime && runtime.chordDetectError) || ""
  };
}
...
function sessionPage(){
  ...
  var displayChord=c;
  var selectedVoicing = sessionStateRead("selectedVoicing", 0);
  var practiceIntention = sessionStateRead("practiceIntention", "");
  if(voicings&&selectedVoicing>0&&selectedVoicing<voicings.length){
    displayChord=voicings[selectedVoicing];
  }
  ...
  if(voicings&&voicings.length>1){
    ...
    for(var i=0;i<voicings.length;i++){
      h+='<button class="voicing-tab'+(selectedVoicing===i?" active":"")+'" onclick="act(\'selectVoicing\',\''+i+'\')">'+voicings[i].label+'</button>';
    }
    h+='</div>';
  }
  ...
  var chordKey=c.name+"_v"+selectedVoicing;
...
function completePage(){
  ...
  var toast = getSessionToastSnapshot();
  var chordProgress = progress.chordProgress || {};
  var n=runtime.chord?runtime.chord.name:"",p=chordProgress[n]||0;
  return '<div class="text-center" style="padding-top:30px"><div style="font-size:56px;margin-bottom:12px;animation:bn .6s ease">&#127881;</div><h2 style="font-size:26px;font-weight:900;color:var(--text-primary)">Awesome!</h2><p style="color:var(--text-dim);font-size:15px;margin-bottom:20px">You practiced <strong>'+n+'</strong></p><div class="card mb20"><div style="display:flex;justify-content:space-around;text-align:center"><div><div style="font-size:28px;font-weight:900;color:#FFE66D">+'+(toast.amount||10)+'</div><div style="font-size:11px;color:var(--text-muted)">XP</div></div><div><div style="font-size:28px;font-weight:900;color:#FF6B6B">&#128293;'+player.streak+'</div><div style="font-size:11px;color:var(--text-muted)">Streak</div></div><div><div style="font-size:28px;font-weight:900;color:#4ECDC4">'+p+'%</div><div style="font-size:11px;color:var(--text-muted)">Mastery</div></div></div></div><div class="flex-col"><button class="btn" onclick="act(\'repeatLegacyPracticeSession\')" style="background:linear-gradient(135deg,#FF6B6B,#FF8A5C);color:#fff">&#128257; One More</button><button class="btn" onclick="act(\'completeSessionHome\')" style="background:#4ECDC4;color:#fff">&#127968; Home</button></div></div>';
}
...
function drillPage(){
  ...
  var drillSwitches = sessionStateRead("drillSwitches", 0);
  var drillAdaptiveBpm = sessionStateRead("drillAdaptiveBpm", 0);
  var activeLevel = sessionStateRead("level", 1);
  ...
  h+='<div><div id="drill-switch-count" style="font-size:32px;font-weight:900;color:#4ECDC4">'+drillSwitches+'</div><div style="font-size:11px;color:var(--text-muted)">switches</div></div>';
  h+='<div style="text-align:center"><div id="drill-adaptive-bpm" style="font-size:18px;font-weight:900;color:#FFE66D">'+drillAdaptiveBpm+'</div><div style="font-size:11px;color:var(--text-muted)">target BPM</div></div></div>';
  h+='<div class="card'+morphClass+'" style="display:inline-block;margin-bottom:12px;border:3px solid '+D.LC[activeLevel]+'">';
  h+='<h3 style="margin:0 0 4px;font-size:16px;color:'+D.LC[activeLevel]+'">'+c.name+tierBadgeHTML(c.name,14)+'</h3>'+UI.chord(c,180,null,drillChanged)+'</div>';
...
function quizPage(){
  ...
  var quizQuestion = sessionStateRead("quizQ", null) || (runtime && runtime.legacyQuizQuestion ? runtime.legacyQuizQuestion : null);
  var quizOptions = Array.isArray(sessionStateRead("quizOpts", [])) && sessionStateRead("quizOpts", []).length ? sessionStateRead("quizOpts", []) : (runtime && Array.isArray(runtime.legacyQuizOptions) ? runtime.legacyQuizOptions : []);
  var quizAnswer = typeof sessionStateRead("quizAns", null) === "string" ? sessionStateRead("quizAns", null) : (runtime ? runtime.legacyQuizAnswer : null);
  var quizScore = typeof sessionStateRead("quizScore", null) === "number" ? sessionStateRead("quizScore", null) : (runtime && typeof runtime.legacyQuizScore === "number" ? runtime.legacyQuizScore : 0);
  var quizTotal = typeof sessionStateRead("quizTotal", null) === "number" ? sessionStateRead("quizTotal", null) : (runtime && typeof runtime.legacyQuizTotal === "number" ? runtime.legacyQuizTotal : 0);
  var quizStreak = typeof sessionStateRead("quizStreak", null) === "number" ? sessionStateRead("quizStreak", null) : (runtime && typeof runtime.legacyQuizStreak === "number" ? runtime.legacyQuizStreak : 0);
...
function strumDetailPage(){
  ...
  var sp = sessionStateRead("selectedStrum", null) || (runtime && runtime.legacyStrumPattern ? runtime.legacyStrumPattern : null); if(!sp)return '';
  var strumActive = typeof sessionStateRead("strumActive", null) === "boolean" ? sessionStateRead("strumActive", null) : !!(runtime && runtime.legacyStrumActive);
  var curBeat = strumActive
    ? (typeof sessionStateRead("_strumBeat", null) === "number" ? sessionStateRead("_strumBeat", null) : (runtime && typeof runtime.legacyStrumBeat === "number" ? runtime.legacyStrumBeat : -1))
    : -1;
  ...
  for(var i=0;i<tones.length;i++){
    var t=tones[i],sel=sessionStateRead("strumTone", null)===t;
...
function songDetailPage(){
  ...
  var sg = songRuntime && songRuntime.songSessionData ? songRuntime.songSessionData : sessionStateRead("selectedSong", null);
  if(!sg)return '';
  var songPlaying = songRuntime && typeof songRuntime.songPlaying === "boolean" ? songRuntime.songPlaying : !!sessionStateRead("songPlaying", false);
  var songBeat = songRuntime && typeof songRuntime.songBeat === "number" ? songRuntime.songBeat : sessionStateRead("songBeat", 0);
...
function songDonePage(){
  var coreView = window.sparkCore && typeof window.sparkCore.getActiveSessionView === "function"
    ...
    : null;
  var songRuntime = coreView && coreView.runtimeState ? coreView.runtimeState : null;
  var selectedSong = sessionStateRead("selectedSong", null);
  var t=songRuntime && songRuntime.songSessionData ? escHTML(songRuntime.songSessionData.title || "") : (selectedSong?escHTML(selectedSong.title):"");
