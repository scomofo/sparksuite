...
var S={
  ...
  performLastTimingOffsetMs:0,
  performLastExpectedLane:null,
...
function resetProgress(){
  ...
  state.currentSession=1;state.guidedSession=1;state.completedSessions=[];state.completedGuidedSessions=[];
  state.guidedPlan=null;state.sessionPlan=null;state.sessionStep=null;state.guidedStep=null;state.guidedPaused=false;
  state.onboardingComplete=false;state.onboardingDone=false;state._inPlacement=false;state._placementIdx=0;
  state.chordProg={};state.lastReviewChords=[];state.lhLevel=1;state.adaptiveBpm=72;state.personalBests={streak:0};
  state.fingerStats={};state.fingerBadges=[];state.fingerExercisesDone=0;state.fingerDaysLogged=0;
  state.earned=[];state.dailyPracticed=0;state.dailyGoal=15;state.songsDone=[];state.totalActions=0;state.jackpotsHit=0;
  ...
  state.dailyChallenges=[];state.weeklyGoals=[];state.skillTree={};state.metaProgress={challengesCompleted:0, goalsCompleted:0, skillPoints:0};state.challengeHistory=[];
  state.analytics={performanceHistory:[], practiceHistory:[], accuracyHistory:[], masteryHistory:[], xpHistory:[], streakHistory:[]};
  state.recommendations=[];state.lastRecommendationRun=null;state.recommendationHistory=[];state.completedLessons=[];
  state.careerProgress={unlockedTiers:{}, unlockedStages:{}, unlockedSongs:{}, songRatings:{}, stageCompletion:{}, tierCompletion:{}};
  state.activeCareerId="career_main";state.activeCareerTier=null;state.activeCareerStage=null;
  state.insightSnapshots=[];state.personalInsights={weakestSkills:[], strongestSkills:[], masteryTrend:{}, practiceTrend:{}, recommendationQuality:{}, careerTrend:{}, packProgress:{}};state.lastInsightRun=null;
  state.challengeRegistry={};state.activeChallenges=[];state.seasonalEvents=[];state.activeEventId=null;
  state.packCompletion={lessons:{}, songs:{}, drills:{}, packs:{}};state.challengeRewards={claimed:{}, eventClaimed:{}, packClaimed:{}};
