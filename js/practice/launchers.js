(function(){
  ...
  function getEffectivePracticeItemType(item){
    ...
    if (type === "practice") {
      ...
      // Engine plans collapse to type "practice"; a bare durationSec is the
      // default warmup item (see selectWarmupCandidate / daily plan generator).
      if (meta.durationSec != null) return "warmup";
    }
  ...
  function isModuleExerciseItem(item){
    ...
    // Engine plans carry the real type in meta.exerciseType and collapse
    // item.type to "practice" via toLegacyPracticePlan.
    var effective = item.meta.exerciseType || item.type;
