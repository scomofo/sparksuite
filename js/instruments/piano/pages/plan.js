...
function pianoPlanPage(){
  ...
  for(var i=0;i<plan.items.length;i++){
    ...
    h += '</div>';
    h += '<button class="btn btn-sm" onclick="launchPracticeItem(sc.r("practicePlan").items['+i+'])" style="background:var(--accent);color:#fff">Go</button>';
    h += '</div>';
    h += '</div>';
  }
