(function() {
  ...
  function summarizeInstrument(name, moduleName) {
    ...
    var chartId = next && typeof mod.selectChartId === "function"
      ...
      : null;
    if (!chartId && mod && typeof mod.getRhythmAdapter === "function") {
      try {
        var rhythm = mod.getRhythmAdapter();
        var payload = rhythm && typeof rhythm.createPayload === "function"
          ? rhythm.createPayload({ curriculum: { nextLessonId: next && next.id }, segment: { meta: { skill: next && next.skill } } })
          : null;
        chartId = payload && payload.chartId;
      } catch (e) {}
    }
    return {
      instrument: name,
