(function() {
  ...
  SparkTransportEngine.prototype.pause = function() {
    this.tick(undefined, { silent: true });
    this._lastTickMs = null;
    this._snapshot = this._createSnapshot({
      status: "paused",
      sourceId: this._snapshot.sourceId,
  ...
  SparkTransportEngine.prototype.tick = function(nowMs, options) {
    options = options || {};
    ...
    this._snapshot = next;
    if (!options.silent) this._emit(this._snapshot.status === "completed" ? "complete" : "tick");
    return this.getSnapshot();
  };
