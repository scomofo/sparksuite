(function() {
  ...
  SparkTimingCore.prototype.validateBeatGrid = function(input) {
    ...
    var issues = [];
    var rawBpm = input.bpm != null ? input.bpm : input.tempo_bpm;
    var bpm = _num(rawBpm, null);
    var rawDownbeats = Array.isArray(input.downbeats_s) ? input.downbeats_s.slice() : [];
    var downbeats = rawDownbeats
      .map(function(value) { return _num(value, null); })
      .filter(function(value) { return value !== null && value >= 0; });

    if (rawBpm != null && rawBpm !== "" && (bpm === null || bpm <= 0)) {
      issues.push({ code: "invalid_bpm", message: "BPM must be greater than zero" });
    }
