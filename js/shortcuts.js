// js/shortcuts.js
// Global keyboard-shortcut handler extracted from js/app.js. Routes
// every relevant keydown event to the legacy `act()` dispatcher (in
// js/actions.js) based on the current screen/tab.
//
// Pure relocation — no behavioral changes. Attaches a single
// document-level keydown listener at script load. Pre-conditions:
// must load AFTER js/state.js (uses S, T, SCR, TAB) and AFTER
// js/actions.js (calls act()) and AFTER js/render.js (calls render()).
