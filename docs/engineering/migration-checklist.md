...
`tests/test_progress_orchestrator_drive.js`. The barrel retirement is done
(item 10 below); the remaining consolidation step is to converge the two
...
3. ✅ InstrumentAdapter normalized methods proxied through spark-core (`getExercisesForLesson`, `getPerformanceConfig`) — `js/sparksuite/bridges/instrument_adapter.js` + `js/sparksuite/legacy/practice_engine.js`; Phase 4 table is Complete for all four instruments.
4. ✅ Recommendation engine wired through SparkCore — `SparkCore.getServices().recommendations` and `SparkCore.recommendNextAction()` (the barrel has since been retired; see item 10) (curriculum queue first, recommendation fallback).
...
9. ✅ Core de-leaked of the flagship instrument hardcodes (CLAUDE.md failure-mode #2). Instrument adapters now declare their specifics via `getCapabilities()`: `sessionStructure` (bass's groove block order) and `skillProgress: { stateKey, movementBasis }` (bass/ukulele rhythm-drill persistence). Both psychology engines resolve session structure from the adapter registry, `progress_engine.js` builds skill-progress patches from the declared config, and `progress_bridge.js` merges any `*SkillProgress` patch key generically. Zero instrument names remain in `psychology_engine.js`, `progress_engine.js`, or `js/sparksuite/services/psychology.js`. The remaining leaks are gone too: `execution_gateway.js` and `session_runtime.js` recognize instrument types via adapter-registry membership instead of a hardcoded shortlist, and `calibration_engine.js` gates the mic-latency offset on the adapter-declared `micCalibration` capability (declared by guitar). Zero instrument-name equality checks remain anywhere in `js/sparksuite/core/`.
10. ✅ Composition roots consolidated. The `js/spark-core/` barrel no longer
   exists: the Phase 7 contract infrastructure was relocated
   (`contracts.js` and `progress-orchestrator.js` → `js/sparksuite/core/`),
   the shared services moved to `js/sparksuite/services/`, and the two true
   legacy engines (`SparkSession`, `SparkPracticeEngine`) live under
   `js/sparksuite/legacy/` and are still loaded for the flows that have not
   migrated. `window.SparkCore` as a namespace barrel is gone. The relocation
   table is in architecture-map.md; the retirement-inventory document that
   tracked this work has been deleted along with the directory it described.

   What remains is a different problem, tracked separately: `spark_core.js`
   is a 143-method god object, and roughly a quarter of `js/sparksuite/core/`
   is not loaded by `index.html` at all. `tests/test_engine_boot_wiring.js`
   now pins which modules are loaded and requires a stated reason for each
   dormant one, so that set can only shrink knowingly.
